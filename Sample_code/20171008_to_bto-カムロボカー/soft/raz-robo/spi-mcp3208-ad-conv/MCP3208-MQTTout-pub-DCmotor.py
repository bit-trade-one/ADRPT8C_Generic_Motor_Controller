...
try:
    while True:
     if __name__ == '__main__':
        ...
        V4norm=V4norm-2048
        V4norm=V4norm/4096
        #print("V4norm="+str(V4norm))
        (rc, mid) = mqttc.publish("In-ch4norm",str(V4norm), qos=2)        
        ...
        V5norm=V5norm-2048
        V5norm=V5norm/4096
        #print("V5norm="+str(V5norm))
        (rc, mid) = mqttc.publish("In-ch5norm",str(V5norm), qos=2)        
        ...
        V6norm=V6norm-900  #1200
        V6norm=V6norm/800
        if  V6norm < -0.5:
            V6norm = 0 
        if  V6norm > 0.5:
            V6norm = 0 
        print("V6norm="+str(V6norm))
        (rc, mid) = mqttc.publish("In-ch6norm",str(V6norm), qos=2)        
        ...
        if  V7norm > 0.5:
            V7norm = 0 
        print("V7norm="+str(V7norm))
        (rc, mid) = mqttc.publish("In-ch7norm",str(V7norm), qos=2)        
        
        #tasks =  [{"ch0":V0,"ch1":V1}]
        tasks =  {"ch0":V0,"ch1":V1,"ch2":V2,"ch3":V3,"ch4":V4,"ch5":V5,
                  "ch6":V6,"ch7":V7,"ch6norm":V6norm,"ch7norm":V7norm} 


        #print(tasks )
        #(rc, mid) = mqttc.publish("In-ch0-7",payload=jsonify({'readings': tasks}) , qos=2)
        (rc, mid) = mqttc.publish("In-ch07",payload= json.dumps(tasks) , qos=2)
